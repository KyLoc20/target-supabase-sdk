...
export function countTasksByType(tasks: Task[], taskType: string, statuses: TaskStatus[]): TaskStatusCount {
...
export function summarizeTaskQueue(tasks: Task[], taskType: string, statuses: TaskStatus[]): number {
    return tasks.filter((task) => task.value === taskType && statuses.includes(task.details.status)).length;
}
